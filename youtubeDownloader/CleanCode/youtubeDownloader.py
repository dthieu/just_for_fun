...
class TabDownload(tk.Frame):
    ...
    def create_gui(self):
        ...
        self.btn_load_stream = tk.Button(self, text="Load stream", command=lambda: Thread(target=self.load_stream).start())
        self.lbl_save        = tk.Label(self, text="Save: ")
        self.txt_save        = tk.Entry(self, width=50)
        self.btn_save_to     = tk.Button(self, text="Browser...")
        self.btn_download    = tk.Button(self, text="Dowload", command=lambda: Thread(target=self.download_stream).start())
        self.btn_cancel      = tk.Button(self, text="Cancel")
        self.lbl_progress    = tk.Label(self, text="Progress:")
        self.pgsbar_progress = ttk.Progressbar(self,
                                                  length=300, 
        ...
        self.cv_thumbnail    = tk.Canvas(self, width=TabDownload.thumbnail_w, height=TabDownload.thumbnail_h, bg='black')
        ...
        self.btn_download.grid(   row=2, column=1, sticky='s', padx=5, pady=3)
        ...
        self.cv_thumbnail.grid(   row=0, column=4, rowspan=4, sticky='w', padx=5, pady=3)
        self.lbl_vidtitle.grid(   row=4, column=4, sticky='w', padx=2, pady=2)
        self.lbl_vidduration.grid(row=5, column=4, sticky='nw', padx=2, pady=2)
    ...
    def bind_func(self):
        # self.btn_load_stream.bind("<ButtonRelease-1>", func=lambda: Thread(target=self.load_stream).start())
        self.btn_save_to.bind("<ButtonRelease-1>", func=self.select_location)
        # self.btn_download.bind("<ButtonRelease-1>", func=lambda: Thread(target=self.download_stream).start())
    ...
    def download_stream(self):
        ...
        if not self.list_streams_video:
            messagebox.showinfo("Empty link", "Please get streams first and try again!")
        
        save_path = self.txt_save.get()
        stream    = None
        
        stream = self.list_streams_video[self.lstbox_streams.index(tk.ANCHOR)]
        # Download video
        try:
